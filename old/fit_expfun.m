...
if(exist(testfile,'file'))
    ...
    for i=1:length(dnr)
        ...
        fidmag10(i)  = fopen([rdir rdate{i} '.mag10'],'a');
        fidt(i)    = fopen([rdir rdate{i} '.time0'],'a');
    end
    for i=1:length(dirs)
        for j=1:length(divs)
            fiddiv(i,j)=fopen([rdir dirs{i} '.shift' num2str(j)],'a');
        end
    end
...
%     oldmags=mags;
     parfor i=1:length(goodid)
        ...
        goodmag=doinv(:,i);
        if(sum(goodmag)>0)
            [newt,res]=lsqnonlin('expfunc',tmpt(goodmag),LB(goodmag),UB(goodmag),options,dn(g),dnr(goodmag),data(g));
            [res2,synth2,test2]=expfunc(newt,dn(g),dnr(goodmag),data(g));
            a=nan(length(dnr),1);
            b=a;
